...
初回登録版はローカル169項目、同一PCの独立2クライアントによる接続・部位破壊・変身・勝利同期を確認済みです。別回線、4人の長時間プレイ、ホスト交代は検証範囲外です。更新後の結果はこのREADMEとdocsに追記します。

現在、物理と遠距離の育成速度、杖のレベルアップ変化、3段階のボス戦、反応・バリアの視認性を改良中です。脳波を使う研究連携は将来案であり、現時点では未実装です。
