...
ホームでキャラクター、ソロマップ（草原／遺跡）、準備時間、討伐対象を選びます。ソロ終了時にビルドを保存し、ロビーから同じ部屋コードで合流します。部屋主がボス戦を開始します。「ボス戦の練習」はオフライン用です。
...
- 草原と遺跡の2マップ。各3種類の区画、つながる街道、障害物、周辺ミニマップ。宝箱は未実装。

- ソロ準備、経験値、レベルアップ3択、時間経過で強くなる敵5種。雑魚も透過アートと3フレームの歩行へ刷新。
- 属性の着弾・斬撃・敵弾に描き込みのある素材を使用。雑魚の攻撃予告は非表示。ボスの危険範囲は表示。
- アーチャー、ウォリアー、ナイト、メイジ、ランサーの5キャラクター。メイジは初期属性を9種類から選択。
- 木＋水の開花から、雷で超開花・炎で烈開花へ派生する3属性連鎖。種は敵ごとにチーム共有1個。
...
- 部位の破壊順で攻撃が分岐。4ボス×3形態の透過アートを刷新。敵の予告と弾を味方エフェクトより前面に表示。
...
最新のマップ実装は[草原と遺跡のマップ](docs/草原と遺跡のマップ.md)、見た目の変更は[雑魚と攻撃のグラフィック](docs/雑魚と攻撃のグラフィック.md)、属性修正は[拡散と結晶の修正](docs/拡散と結晶の修正.md)、ボス戦の実測・検証範囲・残る制限は[部位戦術の検証結果](docs/部位戦術の検証結果.md)、属性連鎖は[3属性連鎖とボス分岐の検証](docs/3属性連鎖とボス分岐の検証.md)、前回の物理調整は[物理合体とバランス検証](docs/物理合体とバランス検証.md)に記載しています。遊びの改善案は[プレイ検証で見つけた改善案](docs/プレイ検証で見つけた改善案.md)、過去版の測定は[変更・検証記録](docs/変更と検証.md)、全クラスの責務は[スクリプト役割一覧](docs/スクリプト役割一覧.md)を参照してください。脳波を使う研究連携は将来案であり、現時点では未実装です。
...
今回の変更・検証条件・今後の提案は[3属性連鎖とボス分岐の検証](docs/3属性連鎖とボス分岐の検証.md)を参照してください。
...
ボス戦ではQ／RB・部位ボタンで8秒間のピンを共有、E／LBで解除できます。破壊後は1.1秒のひるみと4秒の近距離弱点が発生します。操作・倍率・上限は[部位戦術ガイド](docs/部位ピンと破壊チャンス.md)を参照してください。全員が最新版で接続してください。
