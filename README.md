...
最新の属性修正は[拡散と結晶の修正](docs/拡散と結晶の修正.md)、ボス戦の実測・検証範囲・残る制限は[部位戦術の検証結果](docs/部位戦術の検証結果.md)、属性連鎖は[3属性連鎖とボス分岐の検証](docs/3属性連鎖とボス分岐の検証.md)、前回の物理調整は[物理合体とバランス検証](docs/物理合体とバランス検証.md)に記載しています。遊びの改善案は[プレイ検証で見つけた改善案](docs/プレイ検証で見つけた改善案.md)、過去版の測定は[変更・検証記録](docs/変更と検証.md)、全クラスの責務は[スクリプト役割一覧](docs/スクリプト役割一覧.md)を参照してください。脳波を使う研究連携は将来案であり、現時点では未実装です。
